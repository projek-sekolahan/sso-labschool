...
class Ion_auth_model extends CI_Model
{
	...
	public function get_user_by_forgotten_password_code($user_code)
	{
		...
		if($token) {
			...
			if ($user)
			{
				// var_dump($this->verify_password($token->validator, $user->forgotten_password_code));
				// Check the hash against the validator
	...
	public function login($identity, $password)
	{
		...
		$query = $this->db->select($this->tables['users_details'].'.'.$this->identity_column.','.$this->tables['users_login'].'.*')
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->password))
			{
				...
				var_dump($this->set_session($user)); return false;
